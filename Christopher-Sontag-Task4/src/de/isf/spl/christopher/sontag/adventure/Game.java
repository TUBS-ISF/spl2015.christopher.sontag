...
import de.isf.spl.christopher.sontag.adventure.events.Event;
import de.isf.spl.christopher.sontag.adventure.events.Event1;
import de.isf.spl.christopher.sontag.adventure.events.Event2;
import de.isf.spl.christopher.sontag.adventure.events.Event3;
...
public class Game {
	...
	eventsList.add(new Event1());
	eventsList.add(new Event2());
	eventsList.add(new Event3());
	...
	while (!curInput.toUpperCase().equals("EXIT")) {
		...
		if (Config.random.nextInt(50)%4 == 0) {
			Event event = eventsList.get(Config.random.nextInt(eventsList.size()));
			System.out.println("You meet a "+ event.getName());
			event.handle(p);
			if (event.isAffecting()!=null) {
				p.takeDamage(event.isAffecting());
			}
		}
	    }
